...
class ActionDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # TODO: causes different batch sizes with return frame and bb_infos
        ...
        # padding is based on max length of distance vector (hyperparameter?)
        ...
        for region in bb_infos:
            ...
            # to fix error: 
            # Input and output sizes should be greater than 0, but got input (H: 2, W: 0) output (H: 128, W: 128)
            # which is caused by too small bounding boxes ...
            # bounding box should have at least 10 pixel width and height
    ...
    def _drop_irrelevant(self, bb_infos, object_classes):
        for index, obj_class in enumerate(object_classes):
            if obj_class in IRRELAVNT_CLASSES:
                bb_infos.pop(index)
